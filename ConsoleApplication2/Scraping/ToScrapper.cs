        ...
        private async Task GetFlightDetails(JourneyType journeyType, string date)
        {
            ...
            switch (journeyType)
            {
                case JourneyType.OutboundFlight:
                    ...
                    leg = new Leg {From = _tripData.Origin, To = _tripData.Destination, Date = _tripData.OutboundDate};
                    break;
                case JourneyType.InboundFlight:
                    ddd = (string) jObject["SingleDayInbound"];
                    leg = new Leg {From = _tripData.Destination, To = _tripData.Origin, Date = _tripData.InboundDate};

                    break;
            }
        
            _flights.AddRange(new FlightExtractor(ddd,leg).ExtractFlight());
